...
class Scanner(Node):  # MODIFY NAME
    def __init__(self):
        ...
        self.get_logger().info("Initialize the ball-coordinate publisher...")
        self.publisher_ = self.create_publisher(
            msg_type=CameraXY, topic="scanner_coordinates", qos_profile=10)
        self.get_logger().info("done!")

        self.get_logger().info("Initialize the image publisher...")
        self.get_logger().info("done!")

        self.get_logger().info("Starting camera loop...")
        self.camera_loop()

    def publish_topic(self, x, y, x_max, y_max, found, fps):
        msg = CameraXY()
        msg.x = x
        msg.y = y
        msg.x_max = x_max
    ...
    def camera_loop(self):
        ...
        while True:
            ...
            if len(cnts) > 0:
                ...
                # publish on topic
                self.publish_topic(x, (frame.shape[0] - y), frame.shape[1], frame.shape[0], True, fps)
            else:
                self.publish_topic(0.0, 0.0, frame.shape[1], frame.shape[0], False, fps)
