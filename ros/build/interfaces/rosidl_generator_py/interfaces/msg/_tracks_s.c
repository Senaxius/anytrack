...
bool interfaces__msg__tracks__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("interfaces.msg._tracks.Tracks", full_classname_dest, 29) == 0);
  }
  interfaces__msg__Tracks * ros_message = _ros_message;
  {  // header
  ...
  {  // tracks
    ...
    if (!interfaces__msg__Object__Sequence__init(&(ros_message->tracks), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create interfaces__msg__Object__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    interfaces__msg__Object * dest = ros_message->tracks.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!interfaces__msg__object__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * interfaces__msg__tracks__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("interfaces.msg._tracks");
  ...
  {  // tracks
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = interfaces__msg__object__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
